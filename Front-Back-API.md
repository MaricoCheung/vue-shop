...
| 参数名    | 参数说明 | 备注                                                        |
| --------- | -------- | ----------------------------------------------------------- |
| cat_id    | 分类 ID  | 不能为空，如果要添加1级分类，则父分类Id应该设置为  `1`      |
| cat_name  | 分类名称 | 不能为空                                                    |
...
| goods_number | 数量                       |                                                              |
| goods_weight | 重量                       | 不能为空                                                     |
| goods_state  | 商品状态                   | 0: 未通过 1: 审核中 2: 已审核                                |
| add_time     | 添加时间                   |                                                              |
| upd_time     | 更新时间                   |                                                              |
| hot_mumber   | 热销品数量                 |                                                              |
| is_promote   | 是否是热销品               |                                                              |
| pics         | 上传的图片临时路径（对象） | pics_id：图片 ID，goods_id：商品 ID，pics_big：大图，pics_mid：中图，pics_sma：小图 |
| attrs        | 商品的参数（数组）         | goods_id：商品 ID，attr_value：当前商品的参数值，add_price：浮动价格，attr_vals：预定义的参数值，attr_sel：[many, only] |
...
| is_send              | 物流状态     | 可以为空 |
| order_fapiao_title   | 发票抬头     | 可以为空 |
| order_fapiao_company | 实体名称     | 可以为空 |
| order_fapiao_content | 发票内容     | 可以为空 |
| consignee_addr       | 收货地址     | 可以为空 |
...
| order_number         | 订单编号 | 不能为空                                           |
| order_price          | 订单金额 | 不能为空                                           |
| is_send              | 物流状态 | 1:已经发货，0:未发货                               |
| pay_status           | 支付状态 | 0:未付款，1:已付款                                 |
| order_pay            | 支付方式 | 0:未支付，1:支付宝，2:微信，3:银行卡               |
| consignee_addr       | 收货地址 | 字符串String：省市区/县级联数组+详细地址字符串拼接 |
| order_fapiao_title   | 发票抬头 | 字符串String：‘个人‘ ，’公司‘                      |
| order_fapiao_company | 实体名称 | 字符串String                                       |
| order_fapiao_content | 发票内容 | 字符串String                                       |
